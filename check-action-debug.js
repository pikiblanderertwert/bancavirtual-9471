export async function onRequestGet(context) {
  ...
  if (!sessionId) return new Response(JSON.stringify({ action: 'pending', error: 'No sessionId' }), { headers: { 'Content-Type': 'application/json' } });

  const BIN_ID = env.JSONBIN_BIN_ID;
  const API_KEY = env.JSONBIN_API_KEY;
  if (!BIN_ID || !API_KEY) return new Response(JSON.stringify({ action: 'pending', error: 'Missing JSONBIN credentials in this Cloudflare environment' }), { headers: { 'Content-Type': 'application/json' } });

  try {
    const resp = await fetch('https://api.jsonbin.io/v3/b/' + BIN_ID + '/latest', { headers: { 'X-Master-Key': API_KEY } });
    if (!resp.ok) {
      const errText = await resp.text();
      return new Response(JSON.stringify({ action: 'pending', error: 'JSONBin error: ' + resp.status + ' ' + errText }), { headers: { 'Content-Type': 'application/json' } });
    }
    ...
    return new Response(JSON.stringify({ action: 'pending', error: err.toString() }), { headers: { 'Content-Type': 'application/json' } });
  }
}
